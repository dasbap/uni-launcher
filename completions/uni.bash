_uni() {
  ...
  COMPREPLY=( $(compgen -W "--help --list --foreground --dry-run --verbose --add-runner --remove-runner --add-game --add-emu-game --link --unlink --remove-game --set-emu launchers doctor --install --update -u --system --channel --ref stable prerelease development --merge-config --force-config --installed --missing --current --updates --with-installer --with-emu --all --clear-config $runners $games" -- "$cur") )
}
complete -F _uni uni
